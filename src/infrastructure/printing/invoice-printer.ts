...
export class InvoicePrinter {
  ...
  private buildInvoiceHTML(order: OrderDTO & { customerPhone?:string; customerName?:string }) {
    const linesHtml = order.lines.map((l: any)=>`<tr><td>${l.qty}</td><td>${l.name}</td><td style='text-align:right;'>${this.money(l.unitPrice)}</td><td style='text-align:right;'>${this.money(l.lineTotal)}</td></tr>`).join('');
